...
function App() {
  ...
  useEffect(() => {
    if (!clusters.find((c) => c.rank === activeRank))
      setActiveRank(clusters[0].rank);
  }, [clusters, activeRank]);
  ...
  const [customLabels, setCustomLabels] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cp.labels") || "{}");
    } catch {
      return {};
    }
  });
  const setLabel = (rank, label) => {
    ...
    try {
      localStorage.setItem("cp.labels", JSON.stringify(next));
    } catch {}
  };

  // Derived node state map for the active cluster.
  const nodeMap = useMemo(() => deriveNodeMap(data, cluster), [data, cluster]);
  const clusterForRenderer = {
    ...cluster,
    nodes: nodeMap,
    name: clusterLabel(cluster, customLabels),
  };
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === "ArrowRight")
        setActiveRank(clusters[(idx + 1) % clusters.length].rank);
      else if (e.key === "ArrowLeft")
        setActiveRank(
          clusters[(idx - 1 + clusters.length) % clusters.length].rank,
        );
    }
      ...
      <header className="topbar">
        ...
        <div className="topbar-right">
          <div className="topbar-stat">
            ...
            <span className="topbar-stat-val">{data.bracket}</span>
          </div>
          <div className="topbar-stat">
...
function GlobalPvpPanel({ group, onHover, onLeave }) {
  ...
  const onHoverPvp = (e, p) => {
    onHover({
      node: {
        name: p.name,
        desc: `PvP talent · placeholder description for ${p.name}.`,
        maxPoints: 1,
      },
    ...
    <div className="pvp-panel" data-screen-label="PvP talents (global)">
      <div className="pvp-head">
        ...
        <div className="stat">
          ...
          <span>
            same across all clusters · n={group.sample_size.toLocaleString()}
          </span>
        </div>
      </div>
...
function Tooltip({ node, state, x, y }) {
  ...
  let tag = "SKIP";
  if (role === "core")
    tag = rankFlex ? "CORE · RANK-FLEX" : isContested ? "CLUSTER TAKE" : "CORE";
