...
const TRANSLATIONS = {
  en_US: {
    ...
    flex: "Flex",
    global: "GLOBAL",
    same: "same across all clusters",
    pvp: "PvP Talents",
    slot: "SLOT",
    modal: "Modal pick",
    alts: "Alternatives",
    core: "CORE",
    contested: "CLUSTER TAKE",
    skip: "SKIP",
    rankDist: "Rank distribution",
    notTaken: "Not taken in this cluster",
    pickRate: "Pick rate",
    overall: "overall",
    inCluster: "in cluster",
    variance: "Cluster variance",
    tweaks: "Tweaks",
    flexHighlight: "Flex highlight",
    heatmap: "Pick-rate heatmap",
...
const getLocale = () =>
  window.location.pathname.endsWith("_zh.html") ? "zh_CN" : "en_US";
const t = (key) =>
  (TRANSLATIONS[getLocale()] || TRANSLATIONS["en_US"])[key] || key;
window.t = t;
...
function CrumbDrop({ trigger, active, align, children }) {
  return (
    <div
      className={`crumb-drop ${align === "right" ? "crumb-drop-right" : ""}`}
    >
      <span className={`crumb-trigger ${active ? "active" : ""}`}>
        {trigger}
      </span>
...
function App() {
  ...
  const isSelectionComplete =
    navSelection.class && navSelection.spec && navSelection.bracket;
  const isNavigating =
    isSelectionComplete &&
  ...
  useEffect(() => {
    if (majorClusters.length > 0) {
      if (
        activeRank === null ||
        !majorClusters.find((c) => c.rank === activeRank)
      ) {
  ...
  const cluster =
    majorClusters.find((c) => c.rank === activeRank) || majorClusters[0];
  ...
  const handleLeave = () => {
    ...
    const hasPickers =
      currentTip &&
      currentTip.state &&
      currentTip.state.pickers &&
      currentTip.state.pickers.length > 0;
  ...
  const handlePlayerClick = (p) => {
    ...
    if (found) {
      ...
      console.error(
        "Player not found in data.players:",
        p,
        "players count:",
        (data.players || []).length,
      );
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (idx === -1) return;
      if (e.key === "ArrowRight")
        setActiveRank(majorClusters[(idx + 1) % majorClusters.length].rank);
      else if (e.key === "ArrowLeft")
        setActiveRank(
          majorClusters[(idx - 1 + majorClusters.length) % majorClusters.length]
            .rank,
        );
    }
    ...
    <div className="app" data-screen-label="Talent cluster picker">
      <header className="topbar">
        <div className="brand">
          <img
            ...
            style={{
              "--class-color":
                (CLASSES[navSelection.class] &&
                  CLASSES[navSelection.class].color) ||
                "var(--accent)",
            }}
          />
        </div>
        <div className="crumb">
            ...
            <div className="crumb-menu-grid">
              {Object.entries(CLASSES).map(([cls, info]) => (
                ...
                >
                  ...
                  ></span>
                  <span className="cls-name" style={{ color: info.color }}>
                    {t(cls)}
                  </span>
                </a>
              ))}
            ...
            <CrumbDrop
              trigger={
                navSelection.spec ? (
                  ...
                  <span className="select-hint">{t("selectSpec")}</span>
                )
              }
              active={!navSelection.spec}
            >
              <div className="crumb-menu-title">
                {t("spec")} · {t(navSelection.class)}
              </div>
              {(CLASSES[navSelection.class]
                ? CLASSES[navSelection.class].specs
                : []
              ).map((sp) => (
                <a
                  key={sp}
                ...
                >
                  <span
                    className="cls-dot"
                    style={{ background: CLASSES[navSelection.class].color }}
                  ></span>
                  <span>{t(sp)}</span>
                </a>
              ))}
          ...
          {navSelection.spec ? (
            <CrumbDrop
              trigger={
                navSelection.bracket ? (
                  t(navSelection.bracket)
                ) : (
                  <span className="select-hint">{t("selectBracket")}</span>
                )
              }
              active={!navSelection.bracket}
            >
              ...
              {BRACKETS.map((b) => (
                <a
                  ...
                  href={
                    isNavigating
                      ? buildHref(navSelection.class, navSelection.spec, b)
                      : "#"
                  }
                  onClick={(e) => {
                    if (!isNavigating && b === navSelection.bracket)
                      e.preventDefault();
        ...
        <div className="topbar-right">
          <div className="lang-toggle">
            <a
              href={buildHref(
                data.specLabel.class,
                data.specLabel.spec,
                data.bracket,
                "en_US",
              )}
              className={`lang-btn ${getLocale() === "en_US" ? "active" : ""}`}
            >
              EN
            </a>
            ...
            <a
              ...
              className={`lang-btn ${getLocale() === "zh_CN" ? "active" : ""}`}
            >
              ZH
            </a>
          </div>
        ...
        <div className="player-view-banner">
          <div className="player-view-info">
            ...
            <strong style={{ color: "var(--accent)" }}>
              {activePlayer.name}-{activePlayer.realm}
            </strong>{" "}
            · Spec:{" "}
            <strong>
              {activePlayer.spec} {activePlayer.class}
            </strong>{" "}
            · Rating: <strong>{activePlayer.rating}</strong> · Item Level:{" "}
            <strong>{activePlayer.ilvl}</strong>
          </div>
            ...
            >
              ...
              <span className="tab-bar">
                <span
                  ...
                  style={{ width: `${Math.min(100, c.pct * 4)}%` }}
                ></span>
              </span>
            </button>
          ))}
        ...
        <section className="tree-pane">
          <div className="tree-pane-inner">
            <div className="trees-row">
              {(() => {
                const src = window.CLUSTER_DATA.tree || window.TREE;
                const trees = heroTree
                  ? [src.trees[0], heroTree, src.trees[1]]
                  : src.trees;
                return trees;
              })().map((tree) => (
                  ...
                  <div className="tree-header">
                    ...
                    <div className="stat">
                      <span className="num">
                        {tree.nodes.reduce(
                          (s, tn) =>
                            s + (nodeMap[tn.id] ? nodeMap[tn.id].pts || 1 : 0),
                          0,
                        )}
                      </span>{" "}
                      pts
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Cluster signature">
          <TweakToggle
            label="Mark contested takes"
            value={tweaks.showSignature}
    ...
    <div className="pvp-panel" data-screen-label="PvP talents (global)">
      ...
      <div className="pvp-grid">
        ...
        <div className="pvp-alts">
          <div className="pvp-alts-head">
            {t("alts")} · {alts.length}
          </div>
          <div className="pvp-alts-list">
            {alts.map((p) => (
              <div
                className="pvp-alt-row"
                key={p.id}
                onMouseEnter={(e) => onHoverPvp(e, p)}
                onMouseLeave={onLeave}
              >
...
function Tooltip({
  ...
  const left =
    x + 14 + TOOLTIP_W > window.innerWidth
      ? Math.max(8, x - TOOLTIP_W - 14)
      : x + 14;
  const top =
    y + 14 + TOOLTIP_H_EST > window.innerHeight
  ...
  const displayRank = state
    ? rankDist
      ? window.modalRank(state)
      : state.pts
    : 0;
  ...
  return (
    ...
    >
      <div className="tooltip-head">
        {m && m.icon ? (
          <img className="tooltip-icon" src={m.icon} alt="" />
        ) : null}
        <span className="tooltip-name">{node.name}</span>
        <span
          className={`tooltip-tag ${role} ${isContested ? "contested" : ""}`}
        >
          {tag}
        </span>
      </div>
        ...
        <>
          ...
          <div className="tooltip-bar">
            <div
              ...
              style={{ width: `${state.pickRate}%` }}
            ></div>
          </div>
          {state.pickers &&
            state.pickers.length > 0 &&
            state.pickers.length < (sampleSize || 100) && (
              <div className="tooltip-pickers">
                <div className="tooltip-pickers-title">
                  Players picking this:
                </div>
          ...
          {rankDist ? (
            <>
              ...
              {(() => {
                const skipPct = Math.max(
                  0,
                  100 - rankDist.reduce((s, v) => s + v, 0),
                );
