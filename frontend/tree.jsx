...
function NodeShape({ node, x, y, scale = 1, extraClass = "", heat }) {
  const r =
    (node.type === "capstone" ? 20 : node.type === "diamond" ? 16 : 15) * scale;
  if (node.type === "diamond" || node.type === "capstone") {
    return (
      <polygon
...
function NodeIcon({ node, x, y, iconUrl }) {
  ...
  const baseR =
    node.type === "capstone" ? 20 : node.type === "diamond" ? 16 : 15;
  ...
  const size =
    node.type === "diamond" || node.type === "capstone"
      ? baseR * 1.45
      : baseR * 1.7;
...
function PickRateRing({ node, x, y, pickRate, role }) {
  ...
  return (
    <g
      className={`rate-ring rate-ring-${role}`}
      transform={`rotate(-90 ${x} ${y})`}
    >
      <circle cx={x} cy={y} r={r} className="rate-ring-track" fill="none" />
      <circle
        cx={x}
        cy={y}
        r={r}
...
function TalentTree({
  ...
  return (
    <div className="tree-svg-wrap">
      <svg className={svgClass} width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        <g>
        ...
        <g>
          {tree.nodes.map((node) => {
            ...
            const onEnter = (e) =>
              onHover({ node, state: st, x: e.clientX, y: e.clientY });
            ...
            return (
              ...
              >
                ...
                <NodeIcon
                  ...
                  y={y}
                  iconUrl={
                    meta.get(node.spellId) && meta.get(node.spellId).icon
                  }
                />
                {st ? (
                  <PickRateRing
